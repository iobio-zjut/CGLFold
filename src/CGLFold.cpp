...
void getCmap(){
	...
	ofstream output_I6contact( "output_files/I6_contact" );
	string line1;
	while( getline(contact_file, line1) ){
		istringstream line_data( line1 );
		int res1, res2;		
		
		line_data >> res1 >> res2;
	      
		if ( abs(res1 - res2) >= 6 )
			output_I6contact << line1 << endl;
	}
	output_I6contact.close();
	contact_file.close();
	
	//extract top L/2 filtered contact
	contact_file.open("output_files/I6_contact");
	string line2;
	int num_contact( fasta_length / 2 + fasta_length % 2 );
	while( getline(contact_file, line2) ){
	    istringstream line_data( line2 );
	    ContactPair contact;
	...
	contact_file.close();
	system( "rm output_files/I6_contact" );
...
void apply(){
	...
	system("rm S_00000001.pdb score.fsc default.out");
}

void cluster(){
	cout << "#  Cluster by SPICKER." << endl;
